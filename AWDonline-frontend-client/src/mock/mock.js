...
import qs from 'qs';
import { LoginUsers, Users ,Teams, Chals, Instances, Admins ,TeamInfo} from './data/user';
let _Users = Users;
let _Teams = Teams;
let _Chals = Chals;
let _Instances = Instances;
let _Admins=Admins;
let _TeamInfo = TeamInfo;
...
export default {
  ...
  bootstrap() {
    ...
    //获取用户列表
    mock.onGet('/team/list').reply(config => {
      let {name} = config.params;
      let mockUsers = _Teams.filter(user => {
    ...
    mock.onGet('/user/add').reply(config => {
      ...
      mock.onGet('/team/instances').reply(config=>{
          return new Promise((resolve,reject)=>{
              setTimeout(()=>{
                  resolve([200,{
      ...
      mock.onGet('/team/info').reply(config=>{
          return new Promise((resolve,reject)=>{
              resolve([200,{
                  user:{
                      'name':_TeamInfo.name,
                      'nickname':_TeamInfo.nickname,
                      'score':_TeamInfo.score
                  }
              }])
          })
      });
      mock.onPost('/team/edit').reply(config=>{
          let {name,nickname,old_password,password,password_confirm}=qs.parse(config.data);
          return new Promise((resolve,reject)=>{

              if(password==_TeamInfo.password){
                  _TeamInfo.password=password;
                  _TeamInfo.nickname=nickname;
                  _TeamInfo.name=name;
                  resolve([200,{
                      code:200,
                      msg:'更改成功!'
      ...
      mock.onGet('/team/attack').reply(config=>{
          let data=[{
              attacker:1,
              attacked:2,
              chal:1,
